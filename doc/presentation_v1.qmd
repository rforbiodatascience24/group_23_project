---
title: "Analysis of Mice Protein Expression"
author: "Gaia Marraffa, Natalia Najuch, Gioele Tiraboschi"
format: revealjs
editor: visual
---

## **Group 23**

### **Analysis of Mice Protein Expression**

Gaia Marraffa

Natalia Najuch

Gioele Tiraboschi

------------------------------------------------------------------------

## Introduction

\# Topic: Analysis of Mice Protein Expression

Understanding protein patterns and relationships in the dataset.

# Objective:

\<span style="font-size: 12px;"\>Explore data to identify patterns and correlations among proteins.\</span\>

\<span style="font-size: 12px;"\>Apply advanced data analysis techniques for deeper insights.\</span\>

# Approach:

\<span style="font-size: 10px;"\>Data preprocessing and visualization.\</span\>

\<span style="font-size: 10px;"\>Statistical modeling.\</span\>

\<span style="font-size: 10px;"\>Correlation analysis to uncover hidden relationships.\</span\>

------------------------------------------------------------------------

## Materials and methods

### Materials

-   **Data Source**:
    -   Mice Protein Expression Dataset ([Kaggle](https://www.kaggle.com/datasets/ruslankl/mice-protein-expression)).
    -   Contains protein levels from the cortex and hippocampus of mice across different experimental conditions.\
    -   **Features**:
        -   Protein expression levels for 77 proteins.
        -   72 mice, including both control and trisomy cases.
        -   Additional grouping such as genotype and behavior.

### Methods

#### Workflow Overview:

1.  **Data Preparation**:

    -   Import and clean dataset (handle missing values, standardization).

    -   Data Augmentation.

2.  **Modeling**:

    -   Linear regression to test protein relationships

We transformed the values contained in the columns "Genotype", "Treatment" and "Behavior" into binary variables, so that:

-   trisomic mice are assigned 1 and control mice 0;

-   mice treated with memantine are assigned 1 and with saline 0;

-   stimulated mice are assigned 1 and non-stimulated 0 in the new column "Stimulated".

We created 2 new variables:

**MAPK_Activity**

```{r}
#df_aug1 <- df_aug |>
 # mutate(MAPK_Activity = BRAF_N * pERK_N)
```

-   BRAF_N: A kinase that starts the MAPK signaling pathway, often altered in cancers like melanoma.

-   pERK_N: which transmits growth signals in the cell.

We multiplied BRAF_N and pERK_N, and the new variable measures the MAPK signaling pathway's activity, which is often dysregulated in cancer.

**Chromatin_Activation_Index**

```{r}
#df_aug2 <- df_aug1 |>
 # mutate(Chromatin_Activation_Index = AcetylH3K9_N / DYRK1A_N)
```

-   AcetylH3K9_N: A modification that marks active genes by loosening chromatin, allowing gene expression.

-   DYRK1A_N: A kinase that influences gene expression and cell function.

We divide AcetylH3K9_N by DYRK1A_N and this new index should reflect how epigenetic changes and kinase signaling work together to regulate gene expression in response to external signals.

------------------------------------------------------------------------

## Statistical modeling

We grouped the 8 categories: genotype (control = 0 or Ts65Dn = 1), treatment (Memantine = 1 or Saline = 0), and Stimulated (yes = 1, no = 0).

Then we created a linear model between the 2 variables BRAF_N and pERK_N:

```{r}
#df_models <- df_nested |>
 # mutate(model = map(data, 
  #                   ~lm(BRAF_N ~ pERK_N, 
   #                      data = .x))  )

```

------------------------------------------------------------------------

## Analysis 1

------------------------------------------------------------------------

## **Results #1**

------------------------------------------------------------------------

## Results #2

------------------------------------------------------------------------

## Results #3

------------------------------------------------------------------------

## Discussion

------------------------------------------------------------------------

::: {style="text-align: center; font-size: 2em; font-weight: bold;"}
Thanks for your attention!
:::
