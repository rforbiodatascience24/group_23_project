...
protein_data_long <- df |>
  ...
  pivot_longer(-MouseID, names_to = "Protein", values_to = "Expression")

# Heatmap using ggplot
ggplot(protein_data_long, aes(x = Protein, y = MouseID, fill = Expression)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        axis.text.y = element_text(size = 5, angle = 90, hjust = 1)) +
  labs(title = "Heatmap of Protein Expression (All Mice)",
       x = "Proteins", y = "MouseID", fill = "Expression")
...
df <- df_z |>
  mutate(across(ends_with("_N"), ~ (2 * ((. - min(., na.rm = TRUE)) / 
                                         (max(., na.rm = TRUE) - min(., na.rm = TRUE))) - 1)))
...
for (cls in unique_classes) {
  ...
  heatmap_plot <- ggplot(protein_data_long_class, 
                         aes(x = Protein, y = MouseID, fill = Expression)) +
    geom_tile() +
    scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 2, angle = 90),
...
protein_columns <- colnames(df |> select(ends_with("_N")))

trisomy_long <- trisomy_data |>
  select(MouseID, all_of(protein_columns)) |>
  pivot_longer(cols = -MouseID, names_to = "Protein", values_to = "Expression")
...
#### Plots
```{r}
ggplot(trisomy_long, aes(x = Protein, y = MouseID, fill = Expression)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  theme_minimal() +
  theme(
...
ggplot(control_long, aes(x = Protein, y = MouseID, fill = Expression)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  theme_minimal() +
  theme(
...
most_variable_proteins <- df |>
  ...
  pivot_longer(cols = everything(), names_to = "Protein", values_to = "SD") %>%
  arrange(desc(SD)) |>
  slice_head(n = 10)
...
group_means <- df |>
  group_by(Genotype) |>
  summarise(across(ends_with("_N"), mean, na.rm = TRUE)) |>
  pivot_longer(cols = -Genotype, names_to = "Protein", values_to = "MeanExpression")

protein_differences <- group_means |>
  pivot_wider(names_from = Genotype, values_from = MeanExpression) |>
  mutate(Difference = abs(Ts65Dn - Control)) |>
...
```{r}
t_test_results <- df |>
  pivot_longer(cols = ends_with("_N"), names_to = "Protein", values_to = "Expression") |>
  group_by(Protein) |>
  summarise(p_value = t.test(Expression ~ Genotype, data = cur_data())$p.value) |>
  arrange(p_value) |>
  slice_head(n = 10)  
