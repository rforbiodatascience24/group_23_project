...
Finally, we run the PCA, and we tidy it up to then visualize a scree plot, i.e., a graph that displays the eigenvalues of each principal component and helps to determine which ones are more significative.
...
We plot the data in PC coordinates combining them with the original dataset. First, we visualize the different classes, then we separate the data based on different categories (Genotype, Treatment, Stimulated), in order to eventually recognize a significative pattern.
