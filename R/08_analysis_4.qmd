...
<<<<<<< HEAD
This script analyzes the impact of Genotype, Treatment, and Stimulation on MAPK Activity and Chromatin Activation Index (the 2 variables we created in 03_augment.qmd).
=======
...
>>>>>>> fd7ea71e14a32e0ebcf2ae864d64257db09d85f6
...
```{r}
 df_aug |> 
   ggplot(aes(x = MAPK_Activity + 0.001, # to ensure we won't have log(0) 
...
```{r}
df_aug |> 
  ggplot(aes(x = MAPK_Activity + 0.001,  
                     y = Treatment, 
                     fill = Treatment)) + 
  geom_boxplot() + 
  scale_x_log10() + 
...
```{r}
df_aug |> 
  ggplot(aes(x = Chromatin_Activation_Index, 
             y = Genotype, 
             fill = Genotype)) + 
  geom_boxplot() + 
  scale_x_log10() + 
